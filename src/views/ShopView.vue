<script>
  ...
  export default {
    computed: {
      tokens() {
        return useTokenStore().tokens // Hämta tokens
      },
      bonusTypes() {
        return useTokenStore().bonusTypes // Hämta typ av köpfunktioner
      },
    },
    ...
    methods: {
      buyBonus(name) {
        ...
        const bonus = tokenStore.bonusTypes.find((type) => type.name === name)
        // Kolla om du har nog med tokens som räcker för cost
        if (bonus && tokenStore.tokens >= bonus.cost) {
          // Lägg in i din lista med bonusar och dra av kostnad
          tokenStore.bonusList.push(bonus)
          tokenStore.tokens -= bonus.cost
